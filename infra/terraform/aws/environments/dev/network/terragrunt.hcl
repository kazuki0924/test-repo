...
terraform {
  ...
  extra_arguments "load_storage_auto_tfvars_for_terragrunt_test" {
    commands           = ["test"]
    required_var_files = ["${get_terragrunt_dir()}/../storage/.storage.auto.tfvars"]
  }
}
...
dependencies {
  paths = [
    ...
    "../init",
    # access log buckets, etc. needs to be created
    "../storage"
  ]
}
