...
microservices_shared_resources = null

# per_microservice_resourcesを指定しない場合、以下のリソース一式をvar.common.microservice(マイクロサービスが配置されているディレクトリ名)の名前で作成する
# ecs task def, ecs service or scheduler schedule, alb target group, cwlogs log group, ecr repository, cloud map service discovery service
